...
# ----- BigBox Discount Data (from Screenshot image) ----- #
bigbox_data = {
    ('UAE', 1): 17.0,
    ('UAE', 2): 21.0,
    ('UAE', 5): 29.8,
...
def run_streamlit_ui():
    st.title("Shipping Rate Checker")
    country = st.selectbox("Select Country", sorted(set(c for c, _ in docs_data)))
    dtype = st.radio("Document Type", ['docs', 'non-docs'])

    max_weight = 2.0 if dtype == 'docs' else 25.0
    weight = st.number_input("Enter Weight (KG)", min_value=0.5, max_value=max_weight, step=0.5)

    if dtype == 'docs':
        rate = docs_data.get((country, float(weight)))
        discount = None
    else:
        rate = retail_data.get((country, float(weight)))
        discount = bigbox_data.get((country, int(weight)))

    st.write("### Results")
    st.markdown(
        f"<h4>Retail Rate: <span style='color:#4CAF50'>{rate if rate else 'Not found'} USD</span> | "
        f"Discount Rate: <span style='color:#2196F3'>{discount if discount else 'No discount available'}</span></h4>",
